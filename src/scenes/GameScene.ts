...
export default class GameScene extends Phaser.Scene {
    ...
    init(data?: { zone?: number }) {
        ...
        this.phase1Spawned = false;
        this.phase2Spawned = false;
        this.phase3Spawned = false;
        this.isDead = false;
    }
    ...
    create() {
        ...
        // Play In-Game Soundtrack
        if (!this.sound.get('in_game_soundtrack') || !this.sound.get('in_game_soundtrack').isPlaying) {
            this.sound.play('in_game_soundtrack', { loop: true, volume: 0.3 });
        }
        ...
        for (let i = 0; i < 3; i++) {
            // Space them exactly by 800 pixels (the width of each phase's camera bound)
            // This ensures Phase 1, 2, and 3 have the exact same background framing.
            this.add.image(i * 800, 600, 'chao_fundo').setOrigin(0, 1).setScale(bgScale).setScrollFactor(1);
        }
    ...
    createAnimations() {
        if (!this.anims.exists('player_idle')) {
            ...
            this.anims.create({ key: 'player_run', frames: this.anims.generateFrameNumbers('player', { start: 6, end: 11 }), frameRate: 12, repeat: -1 });
            this.anims.create({ key: 'player_jump', frames: this.anims.generateFrameNumbers('player', { start: 12, end: 17 }), frameRate: 10, repeat: 0 });
            this.anims.create({ key: 'player_attack', frames: this.anims.generateFrameNumbers('player', { start: 18, end: 23 }), frameRate: 15, repeat: 0 });
            this.anims.create({ key: 'player_death', frames: this.anims.generateFrameNumbers('player', { start: 24, end: 29 }), frameRate: 8, repeat: 0 });
            ...
            // Aligator animations
            this.anims.create({ key: 'aligator_run_right', frames: this.anims.generateFrameNumbers('aligator', { start: 0, end: 3 }), frameRate: 8, repeat: -1 });
            this.anims.create({ key: 'aligator_run_left', frames: this.anims.generateFrameNumbers('aligator', { start: 4, end: 7 }), frameRate: 8, repeat: -1 });
            this.anims.create({ key: 'aligator_attack', frames: this.anims.generateFrameNumbers('aligator', { start: 8, end: 11 }), frameRate: 10, repeat: 0 });
            this.anims.create({ key: 'aligator_hurt', frames: this.anims.generateFrameNumbers('aligator', { start: 12, end: 15 }), frameRate: 8, repeat: 0 });

            // Bat animations (4x4 grid - 8 frames for flying)
            this.anims.create({ key: 'bat_fly_right', frames: this.anims.generateFrameNumbers('bat', { start: 0, end: 7 }), frameRate: 15, repeat: -1 });
            this.anims.create({ key: 'bat_fly_left', frames: this.anims.generateFrameNumbers('bat', { start: 0, end: 7 }), frameRate: 15, repeat: -1 });
    ...
    spawnSpecificEnemy(x: number, y: number, type: string) {
            ...
            if (type === 'bat') {
                ...
                const batBody = enemy.body as Phaser.Physics.Arcade.Body;
                batBody.setAllowGravity(false);
                enemy.setDisplaySize(128, 128); // Normal size again
                batBody.setSize(200, 100); // Hitbox inside the 421x327 frame
                batBody.setOffset(110, 113); // Center offset
                ...
                if (type === 'jaguar') {
                    ...
                    const jagBody = enemy.body as Phaser.Physics.Arcade.Body;
                    jagBody.setSize(200, 80);
                    jagBody.setOffset(28, 95); // Physics bottom at 175, matching the visual paws!
                    ...
                    enemy.setDisplaySize(160, 160);
                    const aligBody = enemy.body as Phaser.Physics.Arcade.Body;
                    aligBody.setSize(350, 120); // Width, Height
                    aligBody.setOffset(80, 230); // Physics bottom at 350, matching the visual feet!
                    enemy.setData('type', 'aligator');
                    enemy.setData('speed', 100);
                    enemy.setData('health', 120);
                    enemy.setData('damage', 25);
                    enemy.anims.play('aligator_run_left', true);
                } else {
    ...
    update(time: number, delta: number) {
        ...
        if (this.playerHealth <= 0 && !this.isDead) {
            ...
            this.sound.play('character_death', { volume: 0.8 });
    ...
    handlePhaseLogic() {
        if (this.state === GameState.Phase1_Intro) {
            ...
            if (this.cursors.left.isDown || this.cursors.right.isDown || this.cursors.up.isDown || this.cursors.space.isDown || this.keyA?.isDown || this.keyD?.isDown || this.keyW?.isDown) {
                ...
                // Spawn enemies: 1 aligator, 1 bat
                this.time.delayedCall(2000, () => {
                    this.spawnSpecificEnemy(800, 100, 'aligator');
                    this.time.delayedCall(500, () => this.spawnSpecificEnemy(850, 50, 'bat'));
                    this.phase1Spawned = true;
                });
            }
        } else if (this.state === GameState.Phase1_Combat) {
            if (this.phase1Spawned && this.enemies.countActive(true) === 0) {
                // Defeated
                    ...
                    this.time.delayedCall(1500, () => {
                        ...
                        this.cameras.main.once('camerafadeincomplete', () => {
                            ...
                            this.time.delayedCall(2000, () => {
                                ...
                                this.phase2Spawned = true;
                            });
                        });
                    });
                });
            }
        } else if (this.state === GameState.Phase2_Combat) {
            if (this.phase2Spawned && this.enemies.countActive(true) === 0) {
                // Phase 2 Defeated
                this.state = GameState.Phase2_Cleared;
                
                // Show portal visual
                this.portalTrigger = this.add.text(1480, 380, '➡️\nAVANÇAR', { font: 'bold 20px Courier', color: '#00ff00', align: 'center', stroke: '#000', strokeThickness: 4 }).setOrigin(0.5);
                this.tweens.add({
                    targets: this.portalTrigger,
                    x: 1500,
                    duration: 500,
        ...
        } else if (this.state === GameState.Phase2_Cleared) {
            // Check interaction distance to Portal
            if (this.player.x >= 1480) {
                ...
                this.cameras.main.fadeOut(1500, 0, 0, 0);
                this.cameras.main.once('camerafadeoutcomplete', () => {
                    // Reposition to Phase 3
                    this.player.x = 1650;
                    this.cameras.main.setBounds(1600, 0, 800, 600); // Lock to Phase 3 now
                    
                    // Stay black briefly, then fade in
                    this.time.delayedCall(1500, () => {
                        this.cameras.main.fadeIn(1500, 0, 0, 0);
                        this.cameras.main.once('camerafadeincomplete', () => {
                            this.state = GameState.Phase3_Intro;
                            this.events.emit('showPhase', 'FASE 3');
                            
                            this.time.delayedCall(2000, () => {
                                this.state = GameState.Phase3_Combat;
                                // Spawn enemies inside phase 3 constraints (x: 1600-2400)
                                // Spawning 2 jaguars and 2 alligators
                                this.spawnSpecificEnemy(1600, 100, 'aligator');
                                this.spawnSpecificEnemy(2350, 100, 'aligator');
                                this.time.delayedCall(500, () => {
                                    this.spawnSpecificEnemy(1700, 100, 'jaguar');
                                    this.spawnSpecificEnemy(2200, 100, 'jaguar');
                                });
                                this.phase3Spawned = true;
                            });
                        });
                    });
                });
            }
        } else if (this.state === GameState.Phase3_Combat) {
            if (this.phase3Spawned && this.enemies.countActive(true) === 0) {
                // Phase 3 Defeated - Victory!
                this.state = GameState.Finished;
                
                this.events.emit('showPhase', 'VITÓRIA');
                
                this.time.delayedCall(3000, () => {
                    this.scene.start('VictoryScene', { survivalTime: this.survivalTime });
                });
            }
        }
    }
    ...
    handlePlayerMovement() {
        ...
        if ((this.cursors.up.isDown || this.cursors.space.isDown || this.keyW?.isDown) && isGrounded) {
    ...
    handleCombat() {
        ...
        const attackInput = Phaser.Input.Keyboard.JustDown(this.keyX) || (this.input.activePointer.leftButtonDown() && this.input.activePointer.getDuration() < 50);

        if (attackInput && !this.isAttacking && !this.isDead) {
            this.isAttacking = true;
            
            // Play attack sound
            this.sound.play('knife_slice', { volume: 0.6 });
            ...
            this.physics.overlap(this.attackHitbox, this.enemies, (hb, enemy) => {
                ...
                if (en.getData('type') === 'bat') {
                    ...
                    en.setFlipX(facingRight);
                } else if (en.getData('type') === 'jaguar') {
                    en.anims.play('jaguar_hurt', true);
                } else if (en.getData('type') === 'aligator') {
                    en.anims.play('aligator_hurt', true);
                    // The aligator hurt animation faces right natively. If the player hit it from the right (facing left), we should mirror it.
                    en.setFlipX(!facingRight);
                ...
                if (hp <= 0) {
                    ...
                    if (body) body.enable = false; // Disable physics
                    ...
                    if (en.getData('type') === 'boss') {
                        ...
                        // Blinking effect until it disappears
                        this.tweens.add({
                            targets: en,
                            alpha: 0,
                            duration: 250,
    ...
    handleEnemies(dTSec: number, time: number) {
        this.enemies.getChildren().forEach((child) => {
            ...
            if (enemy.getData('dead')) return; // Skip dead enemies
            ...
            if (enemy.x > this.player.x + 20) {
                ...
                if (type !== 'bat' && type !== 'jaguar' && type !== 'aligator' && type !== 'snake') enemy.setFlipX(false);
            } else if (enemy.x < this.player.x - 20) {
                enemy.setVelocityX(speed);
                if (type !== 'bat' && type !== 'jaguar' && type !== 'aligator' && type !== 'snake') enemy.setFlipX(true); // Flip graphic towards player
            }
            ...
            // Aligator logic
            if (type === 'aligator') {
                if (enemy.anims.currentAnim?.key !== 'aligator_hurt' || enemy.anims.currentFrame?.isLast) {
                    if (Math.abs(enemy.x - this.player.x) < 80) { // close enough to attack
                        enemy.anims.play('aligator_attack', true);
                        // Attack animation faces RIGHT natively. 
                        // If it's attacking left (enemy.x > player.x), we MUST flip it horizontally!
                        if (enemy.x > this.player.x) {
                            enemy.setFlipX(true);
                        } else {
                            enemy.setFlipX(false);
                        ...
                        if (enemyBody.velocity.x > 0) {
                            enemy.anims.play('aligator_run_right', true);
                            enemy.setFlipX(false);
                        } else if (enemyBody.velocity.x < 0) {
                            enemy.anims.play('aligator_run_left', true);
                            enemy.setFlipX(false);
